...
class GeneralProductsMaterialsDao
{
    ...
    public function findAllProductsMaterials($id_company)
    {
        ...
        $stmt = $connection->prepare("SELECT p.id_product, p.reference AS reference_product, p.product, m.id_material, m.reference AS reference_material, m.material, m.quantity
                                      FROM products p
