...
class ProductsDao
{
  ...
  public function findAllProductsByCompany($id_company)
  {
    ...
    $stmt = $connection->prepare("SELECT p.id_product, p.reference, p.product, p.product AS descript, p.img, p.quantity, (SELECT IFNULL(SUM(original_quantity), 0) FROM plan_orders WHERE id_product = p.id_product AND status = 'Despacho') AS reserved, p.classification, 'UNIDAD' AS unit
                                  FROM products p
