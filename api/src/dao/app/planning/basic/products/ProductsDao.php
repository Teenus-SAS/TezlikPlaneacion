...
class ProductsDao
{
  ...
  public function findAllProductsByCompany($id_company)
  {
    ...
    $stmt = $connection->prepare("SELECT p.id_product, p.reference, p.product, p.product AS descript, p.img, p.quantity, p.reserved, p.minimum_stock, p.days, p.classification, 'UNIDAD' AS unit
                                  FROM products p
