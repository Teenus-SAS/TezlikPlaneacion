...
class MinimumStockDao
{
    ...
    public function calcStockByMaterial($id_material)
    {
        try {
            $connection = Connection::getInstance()->getConnection();
            $stmt = $connection->prepare("SELECT SUM(IFNULL(pm.quantity, 0) * IFNULL(p.minimum_stock, 0)) AS stock
                                          FROM materials m
                                            LEFT JOIN products_materials pm ON pm.id_material = m.id_material
                                            LEFT JOIN products p ON p.id_product = pm.id_product	
