...
class MinimumStockDao
{
    ...
    public function calcStockByMaterial($id_material)
    {
        try {
            $connection = Connection::getInstance()->getConnection();
            $stmt = $connection->prepare("SELECT SUM((IFNULL(s.max_term, 0) - IFNULL(s.usual_term, 0)) * ((IFNULL(u.jan, 0) + IFNULL(u.feb, 0) + IFNULL(u.mar, 0) + IFNULL(u.apr, 0) + IFNULL(u.may, 0) + IFNULL(u.jun, 0) + IFNULL(u.jul, 0) + IFNULL(u.aug, 0) + IFNULL(u.sept, 0) + IFNULL(u.oct, 0) + IFNULL(u.nov, 0) + IFNULL(u.dece, 0)) / 12) * pm.quantity) AS stock 
