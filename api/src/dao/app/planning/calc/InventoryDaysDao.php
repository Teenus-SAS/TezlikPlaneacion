    ...
    public function calcInventoryProductDays($id_product)
    {
        try {
            ...
            $stmt = $connection->prepare("SELECT (pi.quantity / ((IFNULL(u.jan, 0) + IFNULL(u.feb, 0) + IFNULL(u.mar, 0) + IFNULL(u.apr, 0) + IFNULL(u.may, 0) + IFNULL(u.jun, 0) + IFNULL(u.jul, 0) + IFNULL(u.aug, 0) + IFNULL(u.sept, 0) + IFNULL(u.oct, 0) + IFNULL(u.nov, 0) + IFNULL(u.dece, 0)) / 
                                                 ...
                                                 (SELECT days FROM sales_days WHERE month = MONTH(CURRENT_DATE()) AND year = YEAR(CURRENT_DATE()) AND id_company = pi.id_company)) AS days
                                      FROM inv_products pi
                                      LEFT JOIN sales_by_units u ON u.id_product = pi.id_product
            ...
            $stmt = $connection->prepare("SELECT 
                                            IFNULL(
                                                SUM(
                                                    ...
                                                    (
                                                        SELECT days FROM sales_days WHERE month = MONTH(CURRENT_DATE()) AND year = YEAR(CURRENT_DATE()) AND id_company = m.id_company
                                                    )
                                                )
                                            , 0) AS days
