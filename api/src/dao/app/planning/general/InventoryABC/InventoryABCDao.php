...
class InventoryABCDao
{
    ...
    public function findAllInventoryABCByComapny($id_company)
    {
        ...
        $stmt = $connection->prepare("SELECT * FROM inventory_abc WHERE id_company = :id_company");
    ...
    public function updateInventoryABC($dataInventory)
    {
        ...
        try {
            $stmt = $connection->prepare("UPDATE inventory_abc SET a = :a, b = :b, c = :c WHERE id_inventory = :id_inventory");
            $stmt->execute([
    ...
    //     $connection = Connection::getInstance()->getConnection();
    //     try {
    //         $stmt = $connection->prepare("SELECT * FROM inventory_abc WHERE id_inventory = :id_inventory");
    //         $stmt->execute(['id_inventory' => $id_inventory]);
    //         $rows = $stmt->rowCount();

    //         if ($rows > 0) {
    //             $stmt = $connection->prepare("DELETE FROM inventory_abc WHERE id_inventory = :id_inventory");
    //             $stmt->execute(['id_inventory' => $id_inventory]);
    //         }
    //         $this->logger->info(__FUNCTION__, array('query' => $stmt->queryString, 'errors' => $stmt->errorInfo()));
    //     } catch (\Exception $e) {
