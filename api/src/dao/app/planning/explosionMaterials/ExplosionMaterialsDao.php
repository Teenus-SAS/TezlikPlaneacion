...
class ExplosionMaterialsDao
{
    ...
    public function findAllMaterialsConsolidated($id_company)
    {
        ...
        $stmt = $connection->prepare("SELECT p.id_product, p.reference AS reference_product, p.product, m.id_material, m.reference AS reference_material, m.material, 
                                             ((SELECT SUM(cpm.quantity) FROM products_materials cpm INNER JOIN plan_orders co ON co.id_product = cpm.id_product WHERE cpm.id_material = m.id_material) * (SELECT SUM(o.original_quantity) FROM plan_orders o INNER JOIN products_materials pm ON pm.id_product = o.id_product WHERE pm.id_material = m.id_material)) AS quantity
                                      FROM products p
