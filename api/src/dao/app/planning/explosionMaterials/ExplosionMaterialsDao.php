...
class ExplosionMaterialsDao
{
  ...
  public function findAllMaterialsConsolidated($id_company)
  {
    ...
    $stmt = $connection->prepare("SELECT p.id_product, o.id_order, pm.id_product_material, p.reference AS reference_product, p.product, SUM(pi.quantity) AS quantity_product, m.id_material, m.reference AS reference_material, m.material, m.quantity AS quantity_material, u.abbreviation, 
                                         IFNULL(SUM(IF(IFNULL(r.admission_date, 0) = 0 AND r.application_date != '0000-00-00' AND r.delivery_date != '0000-00-00', r.quantity_requested, 0)), 0) AS transit, (o.original_quantity * pm.quantity) AS need, m.minimum_stock
  ...
  public function findAllMaterialsConsolidatedByProduct($id_product)
  {
    ...
    $stmt = $connection->prepare("SELECT o.id_order, pm.id_product_material, p.reference AS reference_product, p.product, SUM(pi.quantity) AS quantity_product, m.id_material, m.reference AS reference_material, m.material, m.quantity AS quantity_material, u.abbreviation, 
                                         IFNULL(SUM(IF(IFNULL(r.admission_date, 0) = 0 AND r.application_date != '0000-00-00' AND r.delivery_date != '0000-00-00', r.quantity_requested, 0)), 0) AS transit, (o.original_quantity * pm.quantity) AS need, m.minimum_stock
                                      FROM products p
                                        INNER JOIN products_inventory pi ON pi.id_product = p.id_product
                                        INNER JOIN products_materials pm ON pm.id_product = p.id_product
